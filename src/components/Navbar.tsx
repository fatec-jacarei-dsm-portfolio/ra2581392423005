...
export function Navbar({ isAdmin, onAdminClick }: NavbarProps) {
	...
	return (
		<header
			className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${scrolled ? "bg-[#0d1b2e]/90 backdrop-blur-md border-b border-white/[0.06] py-3" : "py-5"}`}
		>
				...
				<div className="hidden md:flex items-center gap-1">
					...
					{isAdmin && (
						<button
							...
							className="ml-2 flex items-center gap-1.5 bg-indigo-600/20 border border-indigo-500/30 text-indigo-300 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-indigo-600/30 transition"
						>
